...
To update the database use this command from the Web project folder (replace `csumathboy.CleanArch` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../csumathboy.CleanArch.Infrastructure/csumathboy.CleanArch.Infrastructure.csproj -s csumathboy.CleanArch.Web.csproj
```
